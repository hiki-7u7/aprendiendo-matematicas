...
function generatePattern() {
  // Genera un número aleatorio entre 1 y 20 como punto de partida.
  const start = Math.floor(Math.random() * 20) + 1;

  // Construye un patrón simple decreciente de 1 en 1 con 5 números.
  const pattern = [start];
  for (let i = 1; i < 10; i++) {
    const nextNumber = (start - i) % 20;
    pattern.push(nextNumber <= 0 ? 20 + nextNumber : nextNumber);
  }

  return pattern;
}

export function Ejercicio_1_32() {
      ...
      <div className={`min-h-screen ${showMessage ? "hidden" : ""}`}>
        ...
        >
          <h1 className="text-3xl font-semibold">
            Ejercicio 6: Patrones Numéricos del 20 al 1
          </h1>
        </div>
