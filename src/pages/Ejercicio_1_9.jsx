...
import { BiCircle } from "react-icons/bi"; // Icono de circulo
...
export function Ejercicio_1_9() {
  const [numerosRandom, setNumerosRandom] = useState([]); // Números aleatorios
  const [imageneSeleccionadas, setImageneSeleccionadas] = useState([]); // Imágenes seleccionadas
  const [numeroSeleccionado, setNumeroSeleccionado] = useState([]); // Número seleccionado
  const [botonesSeleccionados, setBotonesSeleccionados] = useState([]); // Botones seleccionados
  ...
  const [mensajeColor, setMensajeColor] = useState(null); // Nueva variable de estado para el color del mensaje
  const [showmensaje, setShowmensaje] = useState(false); // Nuevo estado para mostrar el mensaje
  ...
  useEffect(() => {
    const newnumerosRandom = Array.from(
      { length: generateRandomNumber() },
      (_, i) => i + 1
    );
    setNumerosRandom(newnumerosRandom);
  }, []);

  const toggleNumeroSeleccionado = (numero) => {
    if (numeroSeleccionado.includes(numero)) {
      // Deseleccionar el número si ya está seleccionado
      const updatedNumeros = numeroSeleccionado.filter((n) => n !== numero);
      setNumeroSeleccionado(updatedNumeros);
    } else {
      // Seleccionar el número si no estaba seleccionado
      setNumeroSeleccionado([...numeroSeleccionado, numero]);
    }
  };

  const RevisarRespuesta = () => {
    if (
      numeroSeleccionado.length === numerosRandom.length &&
      imageneSeleccionadas.length === numerosRandom.length &&
      respuestasCorrectasSeguidas < 3
    ) {
      setRespuestaCorrecta(true); // Respuesta correcta
      setMensaje("Correcto");
      setMensajeColor("text-green-500"); // Establecer el color del mensaje como verde
      speakText("Correcto");
      ...
      setMensajeColor("text-red-500"); // Establecer el color del mensaje como rojo
  ...
  return (
    <div className="bg-blue-200">
      ...
      <div className={`min-h-screen ${showmensaje ? "hidden" : ""}`}>
        <div
          className=" text-gray-900  py-8 text-center mt-10"
          style={{ backgroundColor: "#FF5C5C" }}
        >
          <h1 className="text-3xl font-semibold">
            Ejercicio 2: Contar y Marcar las Imágenes del 1 al 10
          </h1>
        </div>
        ...
        <div className="container mx-auto mt-8 p-4 text-center">
          <div className="flex justify-center items-center">
            <button
              onClick={() => {
                speakText(
                  "Selecciona las imágenes y marca un circulo por cada imagen."
                );
              }}
          ...
          <div className="mb-6">
            {numerosRandom.map((num) => (
              <div key={num} className="inline-block mx-2">
                <img
                  src={perro}
                  alt={`${num}`}
                  className={`inline-block cursor-pointer rounded-xl ${
                    imageneSeleccionadas.includes(num)
                      ? "border-4 border-blue-500 rounded-xl"
                      : ""
                  }`}
                  onClick={() => {
                    if (!imageneSeleccionadas.includes(num)) {
                      const updatedImages = [...imageneSeleccionadas, num];
                      setImageneSeleccionadas(updatedImages);
                      speakText(imageneSeleccionadas.length + 1);
                    }
                  }}
                />
              </div>
          ...
          <div className="mb-4">
            {Array.from({ length: 10 }, (_, i) => (
              <div key={i + 1} className="inline-block mx-2 rounded-xl ">
                <BiCircle
                  className={`inline-block cursor-pointer text-4xl rounded-full ${
                    numeroSeleccionado.includes(i + 1) ? "bg-red-500" : ""
                  }`}
                  onClick={() => {
                    toggleNumeroSeleccionado(i + 1);
                    speakText(i + 1);
                  }}
                />
              </div>
      ...
      {showmensaje && (
        ...
        >
          <div className="bg-white p-6 rounded-lg shadow-lg">
            <span className={`text-4xl ${mensajeColor ? mensajeColor : ""}`}>
              {mensaje}
            </span>
          </div>
        </div>
