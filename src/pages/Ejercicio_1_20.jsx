...
import Casa from "../assets/img/icono_casa.png";
import Arbol from "../assets/img/icono_arbol.png";
import { useAuth } from "../context/authContext";
import { useNavigate } from "react-router-dom";

export function Ejercicio_1_20() {
  const [pregunta, setPregunta] = useState(
    "¿El árbol está a la derecha o a la izquierda de la casa?"
  );
  ...
  const [arbolEnIzquierda, setArbolEnIzquierda] = useState(true);
  const navegar = useNavigate();
  const { user } = useAuth();

  const cambiarPosicionYRespuesta = () => {
    const posicionAleatoria = Math.random() < 0.5;
    setArbolEnIzquierda(posicionAleatoria);
  ...
  const handleRespuesta = (respuesta) => {
    ...
    if (respuesta === respuestaCorrecta && respuestasCorrectas <= 2) {
      ...
      setPregunta("Vuelve a intentarlo.");
  ...
  const siguientePregunta = () => {
    if (intentos < 2 && respuestasCorrectas < 2) {
      ...
      setUserRespuesta("");
      setIntentos(0);
      setPregunta("¿El árbol está a la derecha o a la izquierda de la casa?");
      speakText(pregunta);
    }
  };
      ...
      <div className="min-h-screen">
        ...
        >
          <h1 className="text-3xl font-semibold">
            Ejercicio 2: ¿Derecha o Izquierda? Ubica el Árbol con Respecto a la
            Casa
          </h1>
        </div>
        <div className="container mx-auto mt-8 p-4 text-center">
          ...
          <div className="flex justify-center mb-4">
            {arbolEnIzquierda ? (
              <>
                <img src={Arbol} alt="Árbol" className="mb-4" />
                <img src={Casa} alt="Casa" className="" />
              </>
            ) : (
              <>
                <img src={Casa} alt="Casa" className="" />
